...
func mapToString(m map[string]nucleo.Payload, ident string) string {
...
func arrayToString(arr []nucleo.Payload, ident string) string {
...
func (p *RawPayload) Bson() bson.M {
	...
	if p.IsMap() {
		bm := bson.M{}
		p.ForEach(func(key interface{}, value nucleo.Payload) bool {
			skey := key.(string)
			if value.IsArray() {
...
func (p *RawPayload) BsonArray() bson.A {
	...
	if p.IsArray() {
		...
		p.ForEach(func(index interface{}, value nucleo.Payload) bool {
			if value.IsMap() {
...
func (p *RawPayload) getPath(path string, defaultValue ...interface{}) nucleo.Payload {
...
func (p *RawPayload) getKey(path string, defaultValue ...interface{}) nucleo.Payload {
...
func (p *RawPayload) Remove(fields ...string) nucleo.Payload {
	...
	if p.IsArray() {
		arr := p.Array()
		new := make([]nucleo.Payload, len(arr))
		for index, item := range arr {
...
func New(source interface{}) nucleo.Payload {
	pl, isPayload := source.(nucleo.Payload)
	if isPayload {
		return pl
	}
