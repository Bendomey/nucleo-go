...
type messageHandlerFunc func(message nucleo.Payload)
...
func createTransit(broker *nucleo.BrokerDelegates) transit.Transit {
	transit := pubsub.Create(broker)
	return transit
}

// createStrategy create a strategy instance based on the config.
func createStrategy(broker *nucleo.BrokerDelegates) strategy.Strategy {
...
func (registry *ServiceRegistry) heartbeatMessageReceived(message nucleo.Payload) {
...
func (registry *ServiceRegistry) remoteNodeInfoReceived(message nucleo.Payload) {
...
func (registry *ServiceRegistry) subscribeInternalEvent(event service.Event) {
	registry.broker.Bus().On(event.Name(), func(data ...interface{}) {
		...
		event.Handler()(eventContext.(nucleo.Context), params)
	})
}
...
func (registry *ServiceRegistry) nextAction(actionName string, strategy strategy.Strategy, opts ...nucleo.Options) *ActionEntry {
